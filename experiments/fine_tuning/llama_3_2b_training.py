...
with open("experiments/create_qa_datasets/final_questions_answers.json", "r") as f:
    questions_answers = json.load(f)

conversations = []

for qa_obj in questions_answers:
    current_conversation = []
    current_conversation += qa_obj["document_messages"]
...
# dataset = load_dataset("./final_qa_dataset", split="train")
...
model.save_pretrained_merged("model", tokenizer, save_method = "merged_16bit",) # Can use save_pretrained_gguf, contains both
...
# Will contain both the gguf file for ollama and the merged model for vllm
