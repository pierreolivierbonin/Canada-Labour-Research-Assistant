...
from tools import retrieve_database_local_chunks_only
import time
...
def main():
    ...
    filename = "new_questions_source_documents.json"
    ...
    for x, question_obj in enumerate(questions_answers):
        ...
        _, _, chunks = retrieve_database_local_chunks_only(question, "en", "labour", chat_model=ChatbotInterfaceConfig.default_model_local, hyperparams=OllamaRAGConfig.HyperparametersAccuracyConfig, engine=engine, n_results=5, is_remote=False, hardcode_question_section_numbers=[section_number], include_html_in_citations=False)
