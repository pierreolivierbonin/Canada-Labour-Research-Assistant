...
def call_llm(client, prompt, model='gemma3n:latest'):
...
if __name__ == "__main__":
    ...
    validated_new_test = UserInput.model_validate_json(new_test2)

    # Create prompt with user data and expected JSON structure
    prompt = f"""
    Please analyze this user query\n {validated_new_test.model_dump_json(indent=2)}:
    ...
    response = call_llm(client=chat, prompt=prompt)
    print(f"\n\n================LLM RESPONSE================\n{response}")