...
4. Creates collections/transport.json with the structured data
...
def extract_transport_acts_and_regulations(blacklist):
    ...
    with safe_beautifulsoup(response.content) as soup:
        ...
        for ul in div.find_all('ul'):
            ...
            for a in ul.find_all('a', href=True):
                ...
                if href and href not in blacklist:
    ...
    # Create transport database configuration
    transport_dict = {
        "name": "transport",
        "languages": ["en"],
        "ressource_name": {
    ...
    for link in all_links:
        ...
        if any(link.startswith(page) for page in law_pages):
            transport_dict["law"]["en"].append(link_id_tuple)
        else:
            not_included_pages.append(link)

    # Save the transport object as JSON
    with open('collections/transport.json', 'w', encoding='utf-8') as f:
        json.dump(transport_dict, f, indent=2, ensure_ascii=False)
    ...
    print("--------------------------------")
    print(f"Created transport database configuration with:")
    print(f"  - {len(transport_dict['law']['en'])} law links")
    print(f"  - Saved to collections/transport.json")
    ...
    return {
        'acts': links_all_transport_acts,
        'regulations': links_all_transport_regulations,
        'all_links': all_links,
        'not_included_pages': not_included_pages,
        'transport': transport_dict
    }

if __name__ == "__main__":
    # Default blacklist includes the L-2 link that was previously hardcoded
    default_blacklist = ["https://laws-lois.justice.gc.ca/eng/acts/L-2"]
    extract_transport_acts_and_regulations(blacklist=default_blacklist)
