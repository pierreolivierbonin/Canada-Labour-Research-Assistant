...
def reprioritize_docs(question_section_numbers, documents, documents_for_messages, metadata, ids, distances):
    if len(question_section_numbers) == 0:
        return documents, documents_for_messages, metadata, ids, distances
    
    prioritized = []
    non_prioritized = []
    
    # Go through list in reverse order
    for doc, doc_for_messages, meta, id, dist in list(zip(documents, documents_for_messages, metadata, ids, distances)):
        ...
        if is_prioritized:
            prioritized.append((doc, doc_for_messages, meta, id, dist))
        else:
            non_prioritized.append((doc, doc_for_messages, meta, id, dist))
    ...
    docs, docs_for_messages, metas, ids, dists = zip(*combined)
    return list(docs), list(docs_for_messages), list(metas), list(ids), list(dists)